...
-(IBAction)btnSave:(id)sender{
    ...
    if (self.caseID != nil && ![self.caseID isEmpty]){
        ...
        AtonementNotice *notice;
        if (noticeArray.count<=0) {
            notice = [AtonementNotice newDataObjectWithEntityName:@"AtonementNotice"];
        }
        if (!notice.caseinfo_id || [notice.caseinfo_id isEmpty]) {
            notice.caseinfo_id = self.caseID;
            [AtonementNoticePrintViewController generateDefaultsForNotice2:notice CASEID:self.caseID];
        }
...
-(NSString*)generateCommonInquireText{
    ...
    text = [NSString stringWithFormat:@"%@%@",text,@"问：司乘人员共多少?\n答：只有一名驾驶员。\n"];
