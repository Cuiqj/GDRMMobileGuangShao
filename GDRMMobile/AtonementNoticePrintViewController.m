...
//    }
    ...
    self.view.frame = CGRectMake(self.view.frame.origin.x, self.view.frame.origin.y, self.view.frame.size.width, 1357);
...
+ (void)generateDefaultsForNotice2:(AtonementNotice *)notice CASEID:(NSString*)caseID{
    ...
    NSRange range = [proveInfo.event_desc rangeOfString:@"于"];
    notice.case_desc = [proveInfo.event_desc substringFromIndex:range.location+1];
    if ([AtonementNoticePrintViewController isTextBeyondRect2:notice.case_desc] == TRUE) {
        NSRange startRange = [proveInfo.event_desc rangeOfString:@"于"];
        NSRange endRange = [proveInfo.event_desc rangeOfString:@"损坏路产如下："];
        NSRange range;
...
//打印数据，仿宋
+ (BOOL)isTextBeyondRect2:(NSString *)content{
    NSError *error=nil;
